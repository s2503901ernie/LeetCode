...
so we never started with numbers like "00", "0.0", "0.00", "1.0", "001", "00.01", or any other number that can be represented with less digits.  
...
class Solution:
    def ambiguousCoordinates(self, s: str) -> List[str]:
        ...
        for i in range(1, len(s)):
            ...
            if left == "" or right == "":
                continue
            left_new = self.get_string(left)
            right_new = self.get_string(right)
            for _left in left_new:
                for _right in right_new:
    ...
    def get_string(self, string):
        ...
        for i in range(1, len(string)):
            new_string = string[0:i] + '.' + string[i:]
            if (i >1 and new_string[0] == '0') or new_string[-1] == '0':
                continue
            else:
